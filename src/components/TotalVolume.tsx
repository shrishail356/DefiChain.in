...
const weiToEth = (wei: string): BigNumber => {
  const ethInWei = new BigNumber('100000000'); // 10^8
...
const TotalVolumeCard: FC<TotalVolumeCardProps> = ({ transactions }) => {
  ...
  const usdValue = ethereumPrice !== null ? totalVolumeEth.multipliedBy(0.26) : null;

  const ethValueString = totalVolumeEth.toFixed(0);
  const ethValueLength = ethValueString.replace('.', '').length;
